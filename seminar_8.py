...
def copy_row_to_file(filename, new_file):
    row_number = int(input('Введите номер строки: '))
    res = read_file(filename)
    ...
    with open(new_file, 'a', encoding='utf-8') as data:
        nf_w = DictWriter(data, fieldnames=['Имя', 'Фамилия', 'Телефон'])
        nf_w.writerow(row_to_copy)
    ...
    while True:
        ...
        if command == 'q':
            ...
            if not exists(filename):
                ...
                continue
            copy_row_to_file(filename, new_file)
